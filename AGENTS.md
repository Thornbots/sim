...
- **The drift suite shares one sim across scenarios (2026-09-23).**
  pytest starts `part:=sim` once; each scenario resets it (`_reset_sim`)
  and launches `part:=robot`, logging to `robot_<n>.log`.
  `--restart-sim` / `restart_sim:=true` is the old fresh-sim-per-scenario
  path. First run (amcl, EKF on, 1x, GUI, 230 s): `baseline` and
  `noise_correction` (1.62) pass, `drift_correction` 0.42 m and
  `drift_correction_obstacle` 0.50 m fail against 0.40 m,
  `jerk_with_motion`'s robot stack never produced `/scan` in 60 s while
  the sim kept publishing `/scan_raw` (cause lost: one `robot.log` was
  overwritten, hence the numbered logs now), and `odom_stuck` failed only
  on an rviz TF-extrapolation `[ERROR]` from the shared sim log, which
  scenarios now skip. `baseline` read `map->odom` exactly 0.0000 m, where
  0.1-0.15 m used to be normal; check amcl had published a real
  correction. Done when all six give the same verdict alone, sixth, and
  under `restart_sim:=true`.
- **Wanted: switch the sim to the newer robot model, `urdf/sentry_v2`,
  for its collision and suspension.** It is generated by
  `tools/simplify_urdf.py` (README.md) and nothing uses it yet;
  `sentry.urdf.xacro` is still what spawns, with no collision on any link
  and no suspension. sentry_v2 has a collision shape per body and sprung
  wheel carriers, which reopens the free-floating `root` question below.
  Its pitch limits, suspension travel, spring rate and damping are
  placeholders.
- **Wanted: a localization scenario with finite acceleration.**
  `drift_harness`'s `drive()` steps `/cmd_vel` to 4 m/s at the start of
  each leg and stops within about one 0.1 s control tick at its end, so
  every scenario corners with effectively infinite acceleration. The real chassis can't do that. Add
