...
- **Wanted: tests against a sim that stays up.** Every drift scenario and
  every `suite:=ekf` run starts gz, the robot and localization from
  scratch, and the stack does not come up cleanly every time. The goal is
  one gz session per test run, with each scenario resetting the robot and
  restarting only the localization stack. The standing rule below (full
  restart before SLAM) exists because partial restarts left stale TF and
  pose state; a persistent sim has to fix that, not ignore it.
