...
def solve_head_angles(target_root):
    ...
    yaw]-> body -[pi yaw]-> headlink(theta_y, axis -z) -> head
    -[HEADPITCH_ORIGIN_YAW yaw]-> headpitch(theta_p, axis y) ->
    head_pitch == camera): the two fixed pi yaws cancel, leaving
    camera_rotation = Rz(HEADPITCH_ORIGIN_YAW - theta_y) @ Ry(theta_p),
    so pointing camera +X at unit direction d=(dx,dy,dz) solves in
    closed form here (numerically verified against cv_target_emulator's
    independent from-scratch FK, see test/test_cv_head_aim.py).
    """
    x, y, z = target_root
    r = math.hypot(x, y)
