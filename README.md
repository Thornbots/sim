...
script's own docstring (trimmed to a short pointer).
...
`colcon test` — fast, single-process, static-analysis checks with no
external state. This suite is the opposite on every axis that matters
for choosing a runner: it needs a running Docker container, gz-sim, and
...
sets up or tears down; each scenario takes real wall-clock seconds to
tens of seconds (physics settling, `minimum_travel_distance`-style
gating, scan-match convergence); scenarios must run strictly
sequentially with a full stack teardown/relaunch between them for clean
map/TF state, which fights `colcon test`'s parallel-by-default model;
and failure diagnosis needs the actual measured drift/correction numbers
printed clearly, not a pytest assert traceback. Nothing here is meant to
run as part of a routine `colcon test` pass — it's invoked deliberately
after tuning `slam.yaml`/`amcl.yaml`/`ekf.yaml`/`pose_emulator.py`'s
noise params — so a plain script is the better fit. It uses `rclpy`
directly (not subprocess+CLI parsing) for all in-process ROS interaction
(TF lookups, service calls, `cmd_vel` publishing).
...
Each backend owns a different TF edge as its "correction" — scenarios
...
- **`slam`** (default): slam_toolbox's own localization mode owns
  ...
  (see `slam.yaml`'s `minimum_travel_distance`) — a jerk with zero
...
3. **drift_correction** — tests lidar relocalization performance against
   ...
   in right as the robot settles, not mid-drive) — it's the correction
   ...
   purpose (see `_run_cornering_loop_scenario`) — comparing the two
...
5. **jerk_with_motion** — (slam/amcl only, see BACKENDS above) models
   ...
   trial over trial. Each trial drives only one short leg — no more
...
`PATROL_LEGS`'s loop stays inside the open central gap the whole time —
...
ramp-adjacent edge (y=-3.35) — this loop never goes south of y=-1.0. Legs
...
to dodge it after live testing showed collisions/overshoot — simpler and
...
`OBSTACLE_XY = (0.5, 0.0)` — shifted 0.5m south of `PATROL_LEGS`'s own
...
`ARCC_Field_2026.sdf` or the saved ARCC26 map — that's the point: from
...
side (unlike earlier abandoned +1/+2m east excursions) — no wall data to
...
`OBSTACLE_LOOP_DWELL_SECONDS = 1.0` — a stationary dwell inserted after
...
the message can't be parsed — keeps this robust to `pose_emulator`
...
process group afterward — no separate despawn needed.
...
trial — `trigger_jerk`'s (dx, dy) is an independent `random.gauss()` draw
...
no added coverage. ALL trials must pass for the scenario to pass — one
...
generous 5s timeout for the same reason — keeps the assertion meaningful
...
snap-back (expected — scan-matching corrects the pose graph
...
(0.5) to make this suite actually exercise realistic conditions — if
...
correction-fraction plateau in either direction — re-derive if this
...
`(jerk_dx, jerk_dy)` — driving the planned leg unmodified from there
...
Plain interface docstrings, no design history — no README entry needed.
